...
class DeviceGroupQuerySpec extends ScalaTestWithActorTestKit with AnyWordSpecLike {
  ...
  "DeviceGroupQuerySpec" must {
    "return temperature value for working devices" in {
      ...
      val queryActor =
        spawn(DeviceGroupQuery(deviceIdToActor = deviceIdToActor, requestId = 1, requester = requester.ref, timeout = 3.seconds))

      device1.expectMessageType[Device.ReadTemperature]
      device2.expectMessageType[Device.ReadTemperature]

      queryActor ! WrappedRespondTemperature(Device.RespondTemperature(requestId = 0, deviceId = "device1", value = Some(1.0)))
      queryActor ! WrappedRespondTemperature(Device.RespondTemperature(requestId = 0, deviceId = "device2", value = Some(2.0)))
    ...
    "return TemperatureNotAvailable for devices with no readings" in {
      ...
      queryActor ! WrappedRespondTemperature(Device.RespondTemperature(requestId = 0, deviceId = "device1", value = None))
      queryActor ! WrappedRespondTemperature(Device.RespondTemperature(requestId = 0, deviceId = "device2", value = Some(2.0)))
